-- search_products_feed: ürün açıklaması ve tags aramasına da bak
-- Bu sayede "beyaz gömlek" gibi sorgular açıklama içeren ürünleri de bulur
...
AS $$
  ...
  ORDER BY
    -- İsimde tam eşleşme önce
    CASE WHEN p.name ILIKE '%' || p_search || '%' THEN 0 ELSE 1 END,
    p.is_on_sale DESC,
    p.created_at DESC
  LIMIT  p_page_size
